...
1. Why is a traditional indexed `for` loop often a weaker default for array iteration than a purpose-built array method? Give two concrete failure/readability reasons.
    You can run into off by one issues, 
    it doesn't account for undefined values by default, and it's too verbose or long handed 
...
2. Explain the difference between `for...of` and `for...in`. What does each iterate over, and which is normally appropriate for array values?
    For of is better suited for objects to iterate through key value pairs
    and for in is better suited for arrays and iterating over indices

3. What do `break` and `continue` do? Why can neither be used to stop a `forEach` early in the same way?
    break terminates after the preceding operation once it's read
    continue allows a program to proceed to execute if the proceeding operation fails
...
4. Predict the result and explain why:
   ...
   4, 8, 12. All values were multplied by 2.

5. Distinguish `some` from `every`. Include what each returns and when each can stop examining the array.
   Returns any value mathching the condition not just the first one and every returns all conditions matching the condition
...
8. Distinguish `find`, `findIndex`, and `filter` by return value and number of matches represented.
    Returns first value in an array that matches the condition.
    Returns the index of the value that matches the condition 
...
9. You only need to know whether an array contains the primitive value `"admin"`. Why might `includes` communicate intent better than `find`?
   You just need to know that it exists not necessarily that it's value exist of the first 
10. What does `map` promise about the relationship between input elements and output elements? Contrast that with `filter`.
   map returns a new array and filter matches values of a condition withn an existing array
...
13. Choose the most expressive tool and justify it:
    - total all invoice amounts; forEach
    - determine whether any user is suspended; includes
    - determine whether all submissions are complete; filter
    - retrieve the first overdue invoice; find
    - retrieve the position of the first overdue invoice; findIndex
    - create an array containing only overdue invoices; map
    - create an array of invoice IDs. map and findIndex
